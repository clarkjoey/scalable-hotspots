...
function drawRectangle(event) {
    if (!isDrawing) return;
    var svgRect = svg.getBoundingClientRect(); // Get the position of the SVG element
    var mouseX = ((event.clientX - svgRect.left)/svgRect.width)*2739; // Calculate the mouse position relative to the SVG element
    var mouseY = ((event.clientY - svgRect.top)/svgRect.height)*2054; // Calculate the mouse position relative to the SVG element
